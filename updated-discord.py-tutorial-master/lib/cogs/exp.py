...
class Exp(Cog):
	...
	async def check_lvl_rewards(self, message, lvl):
		if lvl >= 50: # light green
			if (new_role := message.guild.get_role(834885795566846053)) not in message.author.roles:
				await message.author.add_roles(new_role)
				await message.author.remove_roles(message.guild.get_role(834885789090578474))

		elif 40 <= lvl < 50: # Dark green
			if (new_role := message.guild.get_role(834885789090578474)) not in message.author.roles:
				await message.author.add_roles(new_role)
				await message.author.remove_roles(message.guild.get_role(834885627031846963))

		elif 30 <= lvl < 40: # orange 
			if (new_role := message.guild.get_role(834885627031846963)) not in message.author.roles:
				await message.author.add_roles(new_role)
				await message.author.remove_roles(message.guild.get_role(834885564704489533))

		elif 20 <= lvl < 30: # purple
			if (new_role := message.guild.get_role(834885564704489533)) not in message.author.roles:
				await message.author.add_roles(new_role)
				await message.author.remove_roles(message.guild.get_role(834885554172592158))

		elif 10 <= lvl < 20: # Blue
			if (new_role := message.guild.get_role(834885554172592158)) not in message.author.roles:
				await message.author.add_roles(new_role)
				await message.author.remove_roles(message.guild.get_role(834885517979680778))

		elif 5 <= lvl < 9: # Dark Red
			if (new_role := message.guild.get_role(834885517979680778)) not in message.author.roles:
				await message.author.add_roles(new_role)
	...
	async def on_ready(self):
		if not self.bot.ready:
			self.levelup_channel = self.bot.get_channel(834896336096460860)
